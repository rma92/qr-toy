...
var chunkByteLengths = [];
var chunkEncodingOverride = "";
var bChunksArePrebuilt = false;
var icrc32 = 0;
var bLzma = 0;
...
function scaleCanvasOut()
{
  ...
  //TODO: Change this to use the actual dimensions of the controls.
    xAdjustment = (document.getElementById('controlBox').style.display == 'none')?125:500;
...
function b_crc32_bytes(bytes) {
  var crc = -1;
  for (var i = 0; i < bytes.length; i++) {
    crc = (crc >>> 8) ^ b_table[(crc ^ bytes[i]) & 0xFF];
  }
  return (crc ^ (-1)) >>> 0;
}
...
function estimateChunkByteLength(encodingType, splitSize) {
  var targetChars = Math.max(1, parseInt(splitSize, 10) || 1100);
  if (encodingType === "base10") {
    return Math.max(1, Math.floor((targetChars - 8) / B10CONV_DIGITS_PER_BYTE));
  }
  if (encodingType === "base64") {
    return Math.max(1, Math.floor((targetChars - 8) * 3 / 4));
  }
  return targetChars;
}

function buildChunkLocalFileChunks(fileBytes, encodingType) {
  var bytesPerChunk = estimateChunkByteLength(encodingType, document.getElementById("split_size").value);
  var out = [];
  var lengths = [];
  if (fileBytes.length === 0) {
    return { chunks: [""], lengths: [0] };
  }
  for (var offset = 0; offset < fileBytes.length; offset += bytesPerChunk) {
    var part = fileBytes.subarray(offset, Math.min(offset + bytesPerChunk, fileBytes.length));
    lengths.push(part.length);
    if (encodingType === "base10") {
      out.push(b10encode(part));
    } else if (encodingType === "base64") {
      out.push(uint8ArrayToBase64(part));
    } else {
      out.push(new TextDecoder("utf-8").decode(part));
    }
  }
  return { chunks: out, lengths: lengths };
}
...
function buildQStrForPage(pageIndex) {
  ...
  let fileData = "";
  if (chunkEncodingOverride) {
    fileData = chunkEncodingOverride;
  } else {
  ...
  const extraHeader = chunkByteLengths[pageIndex] != null ? szHeaderSeparator + chunkByteLengths[pageIndex] : "";
  ...
  return (
    ...
    fileData + extraHeader + szHeaderTerminator +
    chunks[pageIndex]
  );
}
...
function makeCodeByChunkId( pageid )
{
  ...
  var extraHeader = chunkByteLengths[pageid] != null ? szHeaderSeparator + chunkByteLengths[pageid] : "";
  qStr = qStrPrefix + szHeaderSeparator + pageid + szHeaderSeparator + chunks.length + szHeaderSeparator + icrc32 + szHeaderSeparator + szFilename + szHeaderSeparator + fileData + extraHeader + szHeaderTerminator + chunks[pageid];
  //qStr = "Q:" + pageid + ":" + chunks.length + ":" + icrc32 + "::" + chunks[pageid];
  if( bQrSplitterDebug ) console.log( "makeNextCode - qString: " + qStr);
  makeCodeInt( qStr );
}

function computeRgbOverlayQrs(qStrR, qStrG, qStrB) {
  const codeMode = document.getElementById('codeMode').value;
  const qStrings = [qStrR, qStrG, qStrB];

  if (codeMode !== 'qr') {
    const qrs = qStrings.map((qStr) => computeQrForPayload(qStr));
    const firstSize = qrs[0] && qrs[0].modules ? qrs[0].modules.length : 0;
    if (!firstSize || qrs.some((qr) => !qr.modules || qr.modules.length !== firstSize)) {
      return null;
    }
    return qrs;
  }
  ...
  const minVersion = parseInt(document.getElementById('iMinVersion').value, 10);
  const maxVersion = parseInt(document.getElementById('iMaxVersion').value, 10);
...
function makeNextCode() {
  ...
  const qrs = computeRgbOverlayQrs(qStrR, qStrG, qStrB);
  if (!qrs) {
    makeCodeByChunkId(pR);
    document.getElementById("pageDataOut").value =
      `RGB overlay fell back to page ${pR}: channels could not share QR size and mask`;
    pageid = pR;
    return;
  }
 ...
 renderRgbComposite(qrR, qrG, qrB, invert);
  ...
  document.getElementById("pageDataOut").value =
    `${pR},${pG},${pB} out of ${chunks.length} v: ${qrR.version} mask: ${qrR.mask} size: ${qrR.modules.length}`;
...
{
  if (!bChunksArePrebuilt) {
    chunks = [];
    chunkByteLengths = [];
    chunkEncodingOverride = "";
  }
  makeCode();
}
...
function ui_loadFileToInput()
{
  ...
  if (file)
  {
    ...
    reader.onload = function(e)
    {
      ...
      const useChunkLocal = document.getElementById("bChunkLocalEncoding") &&
                            document.getElementById("bChunkLocalEncoding").checked &&
                            (encodingType === "base64" || encodingType === "base10");
      if (useChunkLocal)
      {
        if (encodingType === "base64")
        {
        ...
        const built = buildChunkLocalFileChunks(fileContents, encodingType);
        chunks = built.chunks;
        window.chunks = chunks;
        chunkByteLengths = built.lengths;
        chunkEncodingOverride = encodingType === "base64" ? (bLzma ? "LB6C" : "B64C") : (bLzma ? "LB1C" : "B10C");
        bChunksArePrebuilt = true;
        icrc32 = b_crc32_bytes(fileContents);
        pageid = -1;
        document.getElementById('text').value = `[${chunkEncodingOverride}] ${file.name}: ${fileContents.byteLength} bytes in ${chunks.length} chunks`;
        makeCode();
        return;
      }

      bChunksArePrebuilt = false;
      chunkByteLengths = [];
      chunkEncodingOverride = "";
...
document.getElementById('text').addEventListener("blur", function() { bChunksArePrebuilt = false; ui_makeCode(); });
document.getElementById('text').addEventListener("input", function() { bChunksArePrebuilt = false; ui_makeCode(); });
...
document.getElementById('split_size').addEventListener("input", function() {
  if (bChunksArePrebuilt && document.getElementById('fileInput').files[0]) {
    ui_loadFileToInput();
  } else {
    ui_makeCode();
  }
...
document.getElementById('bChunkLocalEncoding').addEventListener('change', ui_loadFileToInput);
