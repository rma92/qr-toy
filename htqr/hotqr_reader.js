 ...
 */
var B10CONV_DIGITS_PER_BYTE_READER = Math.log10(Math.pow(2, 8));
...
function b10encode( arrayBuffer ) {
  ...
  let encodedLength = Math.ceil(byteArray.length * B10CONV_DIGITS_PER_BYTE_READER);
...
{
  let decodedLength = Math.floor(s.length / B10CONV_DIGITS_PER_BYTE_READER);
  let num = BigInt(s);
